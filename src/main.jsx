...
import {Home,Users,TableProperties,ClipboardList,Bot,Settings,Plus,Trash2,Save,Edit3,Printer as PrinterIcon,Sparkles,WifiOff,Wifi,ArrowUpDown,GraduationCap,FileText,Search,UserPlus,ChevronLeft,ChevronRight,BookOpen,CheckCircle2,Clock3,Layers3,Library,Upload,FolderOpen} from 'lucide-react';
...
function App(){
 ...
 useEffect(()=>persist('m2_grade',grade),[grade]); useEffect(()=>persist('m2_section',section),[section]); useEffect(()=>persist('m2_subject',subject),[subject]); useEffect(()=>persist('m2_library_books',libraryBooks),[libraryBooks]);
 ...
 const addLibraryPdf=async(file)=>{
  ...
  try{
   ...
   setLibraryBooks(x=>[{id,title,grade,subject,fileName:file.name,size:file.size,addedAt:new Date().toISOString()},...x]);
   setLibraryTitle('');
 ...
 {tab==='library'&&<section className="panel library-panel"><div className="panelhead"><div><span className="eyebrow">مكتبتي</span><h2>كتبك وموادك في مكان واحد</h2><p>أضف كتب PDF واربط كل كتاب بالصف والمادة.</p></div></div><div className="library-add"><div className="library-fields"><input value={libraryTitle} onChange={e=>setLibraryTitle(e.target.value)} placeholder="اسم الكتاب (اختياري)"/><select value={grade} onChange={e=>setGrade(e.target.value)}>{grades.map(g=><option key={g}>{g}</option>)}</select><select value={subject} onChange={e=>setSubject(e.target.value)}>{subjects.map(s=><option key={s}>{s}</option>)}</select></div><input ref={pdfInputRef} className="hidden-file" type="file" accept="application/pdf,.pdf" onChange={e=>{const f=e.target.files?.[0];addLibraryPdf(f);e.target.value=''}}/><button className="primary library-upload" onClick={()=>pdfInputRef.current?.click()}><Upload size={18}/> إضافة كتاب PDF</button></div>{libraryMessage&&<div className="note">{libraryMessage}</div>}<div className="library-list">{libraryBooks.length?libraryBooks.map(b=><article className="library-book" key={b.id}><div className="library-cover"><BookOpen size={24}/></div><div className="library-info"><span>PDF · {b.grade} · {b.subject}</span><h3>{b.title}</h3><small>{b.fileName} · {Math.max(1,Math.round((b.size||0)/1024/1024*10)/10)} MB</small></div><div className="library-actions"><button onClick={()=>openLibraryBook(b)}><FolderOpen size={16}/> فتح</button><button onClick={()=>editLibraryBook(b)}><Edit3 size={16}/> تعديل</button><button className="danger" onClick={()=>removeLibraryBook(b)}><Trash2 size={16}/></button></div></article>):<div className="empty-state"><Library size={28}/><strong>مكتبتك فارغة</strong><span>أضف أول كتاب PDF للبدء.</span></div>}</div><p className="note">الكتب تُحفظ محليًا على هذا الجهاز حاليًا. في المرحلة التالية سنستخدم الكتاب مباشرة في «التحضير الذكي من PDF».</p></section>}
