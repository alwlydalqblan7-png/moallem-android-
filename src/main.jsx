...
import {Home,Users,TableProperties,ClipboardList,Bot,Settings,Plus,Trash2,Save,Edit3,Printer as PrinterIcon,Sparkles,WifiOff,Wifi,ArrowUpDown,GraduationCap,FileText,Search,UserPlus,ChevronLeft,ChevronRight,BookOpen,CheckCircle2,Clock3,Layers3,Library,Upload,FolderOpen,CalendarDays,Copy,RefreshCw} from 'lucide-react';
...
const scienceTotalColumns=[{id:'science_biology',name:'علم الأحياء والأرض',max:200,type:'فرع العلوم'},{id:'science_physics',name:'الفيزياء',max:120,type:'فرع العلوم'},{id:'science_chemistry',name:'الكيمياء',max:80,type:'فرع العلوم'}];
...
const extractPdfText=async(bookId,startPage=1,endPage=startPage)=>{
 ...
 return {pageCount:pdf.numPages,startPage:from,endPage:to,text:pages.join('\n\n')};
};
...
function App(){
 ...
 const [savedPreparations,setSavedPreparations]=useState(()=>load('m2_saved_preparations',[]));
 ...
 const [weeklySchedule,setWeeklySchedule]=useState(()=>load('m2_weekly_schedule',{}));
 ...
 const availableSubjects=useMemo(()=>GENERAL_SCIENCE_GRADES.includes(grade)?subjects.filter(s=>!['الفيزياء','الكيمياء','علم الأحياء','علم الأرض'].includes(s)):subjects,[subjects,grade]);
 const bookKey=`${grade}|${section}|${subject}`; const book=gradebooks[bookKey]||{columns:(subject==='العلوم العامة'&&GENERAL_SCIENCE_GRADES.includes(grade)?scienceTotalColumns:seedColumns),marks:{}};
 ...
 const addWeeklyPeriod=day=>setWeeklySchedule(x=>({...x,[day]:[...(x[day]||[]),{id:Date.now(),grade,section,subject,time:''}]}));
 const updateWeeklyPeriod=(day,id,patch)=>setWeeklySchedule(x=>({...x,[day]:(x[day]||[]).map(v=>v.id===id?{...v,...patch}:v)}));
 const removeWeeklyPeriod=(day,id)=>setWeeklySchedule(x=>({...x,[day]:(x[day]||[]).filter(v=>v.id!==id)}));
 ...
 const generateCurriculumPlan=async()=>{
  if(!plannerBook){setLibraryMessage('اختر كتاباً أولاً.');return}
  if(!plannerStart||!plannerEnd){setLibraryMessage('حدد بداية ونهاية الفصل أولاً.');return}
  setPlannerGenerating(true);setLibraryMessage('جاري قراءة فهرس الكتاب وبناء الخطة...');
  try{
   const extracted=await extractPdfText(plannerBook.id,plannerTocStart,plannerTocEnd);
   if(!extracted?.text?.trim())throw new Error('لم يظهر نص قابل للقراءة في صفحات الفهرس المحددة. جرّب صفحات أخرى أو نسخة PDF نصية.');
...
----------------`;
   ...
   const response=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:promptText,context:{grade:plannerBook.grade,section,subject:plannerBook.subject}}),signal:controller.signal});clearTimeout(timer);
   const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data?.error||`خطأ من الخادم (${response.status})`);
   const text=data?.text||data?.answer||data?.result;if(!text)throw new Error('لم يرجع الذكاء خطة صالحة.');
   const plan={id:Date.now(),bookId:plannerBook.id,bookTitle:plannerBook.title,grade:plannerBook.grade,subject:plannerBook.subject,branch:plannerBook.branch||'',start:plannerStart,end:plannerEnd,periods:Number(plannerPeriods)||1,holidays:plannerHolidays.trim(),tocPages:`${extracted.startPage}-${extracted.endPage}`,content:String(text),createdAt:new Date().toISOString()};
   setSavedPlans(x=>[plan,...x.filter(v=>v.bookId!==plannerBook.id)]);setLibraryMessage('تم إنشاء الخطة الفصلية والشهرية والأسبوعية من فهرس الكتاب.');
  }catch(err){setLibraryMessage(err?.name==='AbortError'?'انتهت مهلة إنشاء الخطة. حاول مجدداً.':(err?.message||'تعذر إنشاء الخطة.'))}finally{setPlannerGenerating(false)}
 };
 const printCurriculumPlan=plan=>printDocument(`خطة ${plan.bookTitle}`,`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><style>body{font-family:Arial;padding:28px;line-height:1.9;color:#173653}h1{color:#1c3f66;text-align:center}.meta{background:#f5f7fa;padding:12px;border-radius:10px}pre{white-space:pre-wrap;font-family:Arial}</style></head><body><h1>الخطة التعليمية</h1><div class="meta">${plan.grade} · ${plan.subject}${plan.branch?` · ${plan.branch}`:''}<br>${plan.start} — ${plan.end} · ${plan.periods} حصص أسبوعياً</div><pre>${plan.content.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</pre></body></html>`);
 const prepareLessonFromBook=async()=>{
  if(!plannerBook){setLibraryMessage('اختر كتاباً أولاً.');return}
  setAiLoading(true);setAiError('');setLibraryMessage('جاري قراءة صفحات الكتاب...');
  try{
   const extracted=await extractPdfText(plannerBook.id,bookStartPage,bookEndPage);
   if(!extracted?.text?.trim())throw new Error('لم يتم العثور على نص قابل للقراءة في الصفحات المحددة.');
...
المطلوب إعداد تحضير كامل يشمل: بيانات الدرس، الأهداف، الوسائل، التمهيد، المفاهيم، الشرح خطوة بخطوة، توزيع الحصة أو الحصص، نشاط صفي، 5 أسئلة شفهية مع الإجابات، 5 اختيار من متعدد مع الإجابات، 3 صح أو خطأ مع التصحيح، 3 أسئلة تفكير واستنتاج مع الإجابات، التقويم النهائي، الواجب، ملاحظات للمعلم، وملخص سريع.
إذا كان جزء من المطلوب غير واضح في الصفحات فاذكر ذلك ولا تخترع محتوى.
...
----------------`;
   const endpoint=(import.meta.env.VITE_AI_API_URL||'https://moallem-ai.liondangerous65.workers.dev/api/ai').trim();
   const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),90000);
   const response=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:promptText,context:{grade:plannerBook.grade,section,subject:plannerBook.subject}}),signal:controller.signal});
   clearTimeout(timer);
   const data=await response.json().catch(()=>({}));
   if(!response.ok)throw new Error(data?.error||`خطأ من الخادم (${response.status})`);
   const text=data?.text||data?.answer||data?.result;
   if(!text)throw new Error('وصل رد من خدمة الذكاء بدون نص صالح.');
   setAiResult(String(text));setAiPrompt(`تحضير من كتاب ${plannerBook.title} — ${lessonName}`);
   setLibraryMessage(`تم تحضير ${lessonName} من الصفحات ${extracted.startPage}-${extracted.endPage}.`);setTab('ai');
  }catch(err){const message=err?.name==='AbortError'?'انتهت مهلة الاتصال بالذكاء. حاول مجدداً.':(err?.message||'تعذر تحضير الدرس من الكتاب.');setAiError(message);setLibraryMessage(message)}finally{setAiLoading(false)}
 };
 const saveCurrentPreparation=()=>{
  if(!aiResult.trim())return;
  const title=(bookLessonTitle||aiPrompt||'تحضير درس').trim();
  const item={id:editingPreparationId||Date.now(),title,grade,section,subject,bookId:plannerBook?.id||null,bookTitle:plannerBook?.title||'',content:aiResult,updatedAt:new Date().toISOString()};
  setSavedPreparations(x=>[item,...x.filter(v=>v.id!==item.id)]);setEditingPreparationId(item.id);
 };
 const openPreparation=item=>{setAiResult(item.content);setAiPrompt(item.title);setGrade(item.grade||grade);setSection(item.section||section);setSubject(item.subject||subject);setEditingPreparationId(item.id);setTab('ai')};
 const deletePreparation=id=>{if(confirm('حذف هذا التحضير؟')){setSavedPreparations(x=>x.filter(v=>v.id!==id));if(editingPreparationId===id)setEditingPreparationId(null)}};
 const updateCurrentPreparation=text=>{setAiResult(text);if(editingPreparationId)setSavedPreparations(x=>x.map(v=>v.id===editingPreparationId?{...v,content:text,updatedAt:new Date().toISOString()}:v))};
 const buildPreparationHtml=item=>`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><style>body{font-family:Arial;padding:28px;line-height:1.9;color:#173653}h1{text-align:center;color:#1c3f66}.meta{background:#f5f7fa;padding:12px;border-radius:10px;margin-bottom:16px}pre{white-space:pre-wrap;font-family:Arial;font-size:14px}</style></head><body><h1>${item.title}</h1><div class="meta">${item.grade||''} — الشعبة ${item.section||''} · ${item.subject||''}${item.bookTitle?`<br>الكتاب: ${item.bookTitle}`:''}</div><pre>${item.content.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</pre></body></html>`;
 const printPreparation=item=>printDocument(item.title,buildPreparationHtml(item));
 const aiRun=async()=>{
 ...
 {!reportPreview&&<div className="classbar"><select value={grade} onChange={e=>setGrade(e.target.value)}>{grades.map(x=><option key={x}>{x}</option>)}</select><select value={section} onChange={e=>setSection(e.target.value)}>{sections.map(x=><option key={x}>{x}</option>)}</select><select value={subject} onChange={e=>setSubject(e.target.value)}>{availableSubjects.map(x=><option key={x}>{x}</option>)}</select></div>}
 <main>{reportPreview?<section className="preview-page"><div className="preview-toolbar"><button className="back-button" onClick={()=>setReportPreview(false)}><ChevronRight size={20}/> رجوع</button><div><span className="eyebrow">معاينة الجلاء</span><h2>{grade} — الشعبة {section}</h2><p>{subject}</p></div><button className="primary print-preview" onClick={printReportCards}><PrinterIcon size={17}/> طباعة</button></div><div className="preview-list">{previewStudents.length?previewStudents.map(({student,got,max,pct})=><article className="student-report-preview" key={student.id}><div className="report-badge"><GraduationCap size={22}/></div><div><small>جلاء مدرسي</small><h3>{student.name}</h3><p>{grade} — الشعبة {section} · {subject}</p></div><div className="report-score"><strong>{got}/{max}</strong><span>{pct}%</span></div></article>):<div className="empty-state">لا يوجد طلاب في الشعبة الحالية.</div>}</div><p className="note">زر الرجوع داخل الواجهة وزر Back في Android يعيدانك إلى شاشة التقارير مع بقاء الصف والشعبة والمادة والبيانات كما هي.</p></section>:<>{tab==='home'&&<div className="stack"><section className="hero"><div><span className="eyebrow">معلّم V2.5</span><h2>{grade} — الشعبة {section}</h2><p>{subject} · مساحة يومية واضحة لإدارة الصف بسرعة.</p></div><img src={moallemIcon} alt="شعار معلّم"/></section><section className="summary-grid"><div className="summary-card"><span className="summary-icon blue"><Layers3/></span><div><small>الصفوف</small><strong>{grades.length}</strong></div></div><div className="summary-card"><span className="summary-icon green"><Users/></span><div><small>الطلاب</small><strong>{students.length}</strong></div></div><div className="summary-card"><span className="summary-icon amber"><BookOpen/></span><div><small>المواد</small><strong>{subjects.length}</strong></div></div></section><div className="section-title"><div><span className="eyebrow">اختصارات</span><h3>أدواتك اليومية</h3></div><Clock3 size={20}/></div><div className="grid4"><Tile tone="blue" icon={<Users/>} title="الطلاب" text={`${classStudents.length} طالب`} onClick={()=>setTab('students')}/><Tile tone="cyan" icon={<TableProperties/>} title="دفتر العلامات" text="مرن وقابل للتعديل" onClick={()=>setTab('gradebook')}/><Tile tone="green" icon={<ClipboardList/>} title="الحضور" text="حسب الصف والشعبة" onClick={()=>setTab('attendance')}/><Tile tone="violet" icon={<FileText/>} title="الجلاء والطباعة" text="جاهز للطباعة" onClick={()=>setTab('reports')}/><Tile tone="cyan" icon={<Library/>} title="مكتبتي" text={`${libraryBooks.length} كتاب PDF`} onClick={()=>setTab('library')}/><Tile tone="blue" icon={<CalendarDays/>} title="البرنامج الأسبوعي" text="الأحد إلى الخميس" onClick={()=>setTab('schedule')}/></div><section className="ai-card"><div><span className="ai-icon"><Sparkles/></span><div><strong>المساعد الذكي</strong><span>{online?'متصل وجاهز لتحضير الدروس':'وضع Offline فعال'}</span></div></div><button onClick={()=>setTab('ai')}>فتح المساعد</button></section></div>}
 {tab==='schedule'&&<section className="panel weekly-panel"><div className="panelhead"><div><span className="eyebrow">البرنامج الأسبوعي</span><h2>حصصي من الأحد إلى الخميس</h2><p>أضف وعدّل واحذف الحصص، والحفظ تلقائي على الجهاز.</p></div></div><div className="weekly-days">{WEEK_DAYS.map(day=><article className="weekly-day" key={day}><div className="weekly-day-head"><div><strong>{day}</strong><span>{(weeklySchedule[day]||[]).length} حصة</span></div><button className="primary" onClick={()=>addWeeklyPeriod(day)}><Plus size={16}/> إضافة حصة</button></div><div className="weekly-periods">{(weeklySchedule[day]||[]).length?(weeklySchedule[day]||[]).map((period,i)=><div className="weekly-period" key={period.id}><span className="period-num">{i+1}</span><select value={period.grade} onChange={e=>updateWeeklyPeriod(day,period.id,{grade:e.target.value})}>{grades.map(g=><option key={g}>{g}</option>)}</select><select value={period.section} onChange={e=>updateWeeklyPeriod(day,period.id,{section:e.target.value})}>{sections.map(v=><option key={v}>{v}</option>)}</select><select value={period.subject} onChange={e=>updateWeeklyPeriod(day,period.id,{subject:e.target.value})}>{(GENERAL_SCIENCE_GRADES.includes(period.grade)?subjects.filter(v=>!['الفيزياء','الكيمياء','علم الأحياء','علم الأرض'].includes(v)):subjects).map(v=><option key={v}>{v}</option>)}</select><input type="time" value={period.time||''} onChange={e=>updateWeeklyPeriod(day,period.id,{time:e.target.value})} aria-label={`وقت الحصة ${i+1}`}/><button className="danger" onClick={()=>removeWeeklyPeriod(day,period.id)} aria-label="حذف الحصة"><Trash2 size={16}/></button></div>):<div className="weekly-empty">لا توجد حصص بعد.</div>}</div></article>)}</div></section>}
